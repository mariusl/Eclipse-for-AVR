  ...
  for (unsigned i = 0; i < vars.root_directory_sector_count; ++i)
  {
    if (0 == vars.read_sectors(vars.sector_buffer, vars.root_directory_sector + i, 1))
    {
      for (unsigned j = 0; j < BYTES_PER_SECTOR / 32; ++j)
      {
        ...
        if (0 != (directory_entry->attributes & B00011000))
        {
      ...
      void* locator_data)
{
  ...
  //
  if (0 == memcmp_P(directory_entry, user_data->cooked_name, 11))
  {
     user_data->sector = vars.cluster_2_sector + ((directory_entry->startCluster-2) * vars.sectors_per_cluster);
...
// and the header file.
//
bool microfat2::locateFileStart(const char* cooked_name, unsigned long& file_start_sector, unsigned long& file_size_bytes)
{
